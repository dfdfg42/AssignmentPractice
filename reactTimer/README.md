...
본 프로젝트는 최신 버전의 Node.js 환경에서 정상적으로 실행 가능합니다.

1. **의존성 설치**
   터미널에서 프로젝트 루트 경로(`package.json`이 위치한 곳)로 이동한 후, 아래 명령어를 통해 패키지를 설치합니다.
   ```bash
   npm install
   ```

2. **개발 서버 실행**
   패키지 설치가 완료되면 다음 명령어를 입력하여 로컬 개발 서버를 실행합니다.
   ```bash
   npm run dev
   ```

3. **애플리케이션 접속**
   터미널에 출력되는 로컬 주소(기본값: `http://localhost:5173`)를 복사해 브라우저에 입력하여 타이머 앱을 확인합니다.
